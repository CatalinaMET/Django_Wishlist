...
class UserManager(models.Manager):
    def validador_campos(self, postData):
        ...
        if len(User.objects.filter(username=postData['username'])) > 0:
            ...
            now = date.today()
            doh = datetime.strptime(postData['date_hired'],'%Y-%m-%d').date()
            delta_date = now - doh
...
class ItemManager(models.Manager):
    def validador_campos(self, postData):
        ...
        if len(postData['description'].strip()) < 3:
            errors['description_len'] = "Descripción debe tener al menos 3 caracteres"
...
class Item(models.Model):
    ...
    wishes = models.ManyToManyField(User, related_name="wishes", blank=True)
